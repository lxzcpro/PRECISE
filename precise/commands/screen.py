...
def compute_surface(
    rec_path: Path, rec_name: str, rec_chain: str | None, ply_path: Path
):
    from precise.dataset.precise_dataset import get_data_from_ply
    from precise.preprocess.msms_pipeline import process_protein_auto

    process_protein_auto(rec_path, rec_name, rec_chain, ply_path)
...
def run_virtual_screen(
    ...
    # Step 1: Generate surface (auto-detect chains)
    logger.info(f"Generating surface for {rec_path}")
    surface = compute_surface(rec_path, "RECP", None, out_path / "receptor.ply")
